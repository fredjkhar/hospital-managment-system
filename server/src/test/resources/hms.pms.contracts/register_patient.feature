Feature: Register Patient
  ...
  Scenario: Successful Patient Registration by Authorized Medical Staff
    ...
    And assigns a primary Charge Nurse to the patient based on division
    And sends confirmation message to the medical staff member

  Scenario: Registration with Duplicate Patient Details
    Given a medical staff member is logged in
    And the medical staff member provides invalid or incomplete patient information
